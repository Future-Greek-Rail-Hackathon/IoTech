...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.thing.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.things.update", [$thing->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type_id" id="type_id" required>
                    @foreach($types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('type_id') ? old('type_id') : $thing->type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $thing->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('eui') ? 'is-invalid' : '' }}" type="text" name="eui" id="eui" value="{{ old('eui', $thing->eui) }}" required>
                @if($errors->has('eui'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('installed_at') ? 'is-invalid' : '' }}" type="text" name="installed_at" id="installed_at" value="{{ old('installed_at', $thing->installed_at) }}">
